...
export class ScoutAgent {
    ...
    async startListening() {
        ...
        try {
            ...
            wssClient.watchEvent({
                ...
                onLogs: (logs) => {
                    for (const log of logs) {
                        ...
                        if (log.args.token0)
                            this.scoreLaunch(log.args.token0, false);
                    }
                },
                onError: (error) => {
            ...
            if (this.bot && process.env.TELEGRAM_CHAT_ID) {
                ...
                try {
                    const initMsg = await this.bot.api.sendMessage(chatId, `📡 *Scout Agent WebSocket Started*\n\nStatus: 🟢 Active\nConnection: Real-time (WSS)\nUptime: 0s\nLast Signal: None`, { parse_mode: 'Markdown' });
    ...
    async scoreLaunch(tokenAddress, isGraduated = false) {
        ...
        try {
            ...
            if (deployerHistory) {
                if (deployerHistory.riskScore === 'LOW')
                    score += 20;
                else if (deployerHistory.riskScore === 'MEDIUM')
                    score -= 10;
                else if (deployerHistory.riskScore === 'HIGH')
                    score -= 50; // High risk
            }
            if (holdersData) {
