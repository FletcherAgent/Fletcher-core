 ...
 */
export async function checkLiveness(tokenAddress, tokenData, poolAddress) {
    ...
    if (cached && cached.expiresAt > new Date()) {
        if (cached.verdict === 'DEAD') {
            return { alive: false, failedCheck: cached.failedCheck, failReason: cached.failReason || 'Cached' };
        }
        else {
            return { alive: true };
        }
    ...
    const setCache = async (verdict, failedCheck, failReason) => {
    ...
    const reject = async (check, reason) => {
