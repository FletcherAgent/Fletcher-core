...
function queueLog(emoji, ...args) {
    ...
    if (msg.includes("GMGN API failed"))
        return;
...
bot.command("untrack", async (ctx) => {
    ...
    if (!address)
        return ctx.reply("❌ Usage: `/untrack <address>`", { parse_mode: "Markdown" });
    try {
...
bot.command("grok", async (ctx) => {
    ...
    if (!token)
        return ctx.reply("❌ Usage: `/grok <tokenSymbol>`\\nExample: `/grok PEPE`", { parse_mode: "Markdown" });
...
bot.command("grok_toggle", async (ctx) => {
    ...
    try {
        ...
        ctx.reply(enabled === "true"
            ...
            : "⏸️ **Grok AI Analysis is now DISABLED.** Bot will skip AI checks and auto-approve tokens to save API usage.", { parse_mode: "Markdown" });
    }
    catch (e) {
...
bot.command("wallets", async (ctx) => {
    try {
        ...
        if (wallets.length === 0)
            return ctx.reply("📭 No tracked wallets found.");
...
bot.command("wallet", async (ctx) => {
    const address = ctx.match?.toLowerCase().trim();
    if (!address)
        return ctx.reply("❌ Usage: `/wallet <address>`", { parse_mode: "Markdown" });
    try {
        const w = await prisma.trackedWallet.findUnique({ where: { address } });
        if (!w)
            return ctx.reply(`❌ Wallet \`${address}\` not found in registry.`, { parse_mode: "Markdown" });
        const msg = `
🔍 **Wallet Detail**
- **Address:** \`${w.address}\`
- **Label:** ${w.label || '-'}
...
bot.command("lp", async (ctx) => {
    ...
    if (sub === 'close') {
        const posId = args[1];
        if (!posId)
            return ctx.reply('❌ Usage: `/lp close <position_id>`', { parse_mode: 'Markdown' });
        try {
    ...
    if (data.startsWith('lp_approve:') || data.startsWith('lp_reject:')) {
        ...
        if (isDryRun) {
            ...
            if (type === 'OPEN') {
                ...
                await ctx.editMessageText(`✅ *[DRY RUN] LP OPEN simulated*\nFake tokenId: \`${fakeTokenId}\``, { parse_mode: 'Markdown' });
            }
            else if (type === 'CLOSE') {
