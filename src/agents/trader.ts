import { encodeFunctionData, encodeAbiParameters, decodeAbiParameters, parseAbiParameters, parseAbi, parseEther, concat, toHex, pad, erc20Abi, decodeEventLog, decodeFunctionData } from 'viem';
...
export class TraderAgent {
  ...
  public async processExitSignal(posId: string, tokenAddress: string, amountInToken: bigint, reason: string, txHash?: string) {
    ...
    if (!calldataResult) {
      ...
      const chatId = process.env.TELEGRAM_CHAT_ID;
      if (chatId) this.bot.api.sendMessage(chatId, `❌ **SELL Failed**\nToken: \`${tokenAddress}\`\nReason: Failed to construct transaction (Pool not found or Universal Router parsing failed).`, { parse_mode: 'Markdown' }).catch(console.error);
      return;
    }
    if (calldataResult) {
      ...
      try {
        ...
        if (originalTx && originalTx.to) {
          ...
          if (originalAmountIn > 0n || originalAmountIn === 0n) {
             let modifiedInput: string = this.modifyUniversalRouterCalldata(originalTx.input, amountIn, originalAmountIn);
             if (modifiedInput !== "") {
  ...
  private modifyUniversalRouterCalldata(originalInput: string, newAmountIn: bigint, originalAmountIn: bigint): string {
    ...
    try {
      ...
      try {
        decoded = decodeFunctionData({ abi: [executeAbiV3], data: originalInput as `0x${string}` });
      } catch(e) {
        try {
          decoded = decodeFunctionData({ abi: [executeAbiV2], data: originalInput as `0x${string}` });
      ...
      if (decoded) {
        ...
        const inputs = [...decoded.args[1] as string[]];
        let modified = false;
