...
const ORACLE_PRIVATE_KEY = (process.env.LP_PRIVATE_KEY || process.env.PRIVATE_KEY) as `0x${string}`;

const reputationAbi = parseAbi([
  'function recordPerformance(uint256 agentId, int256 profitUsd, int256 pnl, bool isWin, bytes signature) external'
]);

export async function buildReputationCall(
  agentId: string,
  erc8004Id: string,
  profitUsd: number,
  pnlPercentage: number, // e.g. 5.5 = 5.5%
): Promise<{ target: string; data: Hex } | null> {
  ...
  const messageHash = keccak256(
    encodePacked(
      ['uint256', 'int256', 'int256', 'bool', 'uint256', 'address'],
      [idNum, profitInt, pnlInt, isWin, currentNonce, REPUTATION_REGISTRY_ADDRESS]
    )
  );
