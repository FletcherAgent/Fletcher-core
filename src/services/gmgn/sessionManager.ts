...
export class SessionManager {
  private static session: GMGNSession | null = null;
  private static isRefreshing = false;

  public static async getSession(forceRefresh = false): Promise<GMGNSession> {
    ...
    if (!forceRefresh && !this.session) {
      ...
      if (dbRecord) {
        try {
          const parsed = JSON.parse(dbRecord.value) as GMGNSession;
