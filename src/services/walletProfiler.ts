...
export class WalletProfiler {
   ...
   */
  static async processBuy(walletAddress: string, tokenAddress: string, txHash: string) {
    try {
   ...
   */
  static async processSell(walletAddress: string, tokenAddress: string, txHash: string) {
    try {
      await sleep(3000);
  ...
  private static async updateWalletMetrics(walletAddress: string, pnlRatio: number) {
